...
Module SWPCHEM
    ...
    Real Function Getxx(chem, t, actualTime)
        ! DESCRIPTION:
        !   Gets the density at the given time.
        ! RETURNS:
        !   Temperature at time t.
        ...
        If (i < chem%NPOINTS) Then
            ...
            ! two bounding data points.
            j = i + 1
            Getxx = chem%Chem(chem%Shared%ixx,i) + ((chem%Chem(chem%Shared%ixx,j) - chem%Chem(chem%Shared%ixx,i)) * (t - chem%Times(i)) / &
                                                                                                             (chem%Times(j) - chem%Times(i)))
        Else
            ! This is the last data point, and we don't do
            ! extrapolation.
            Getxx = chem%Chem(chem%Shared%ixx,i)
        End If
    End Function
    ! -------------------------------------------------------
