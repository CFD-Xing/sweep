...
!					particles(2*DoublingLimit-1)%properties = 0.0
!					particles(2*DoublingLimit)%properties = 0.0
...
!					! Copy the particles.
!					particles(HalfCapacity+1:Capacity) = particles(1:HalfCapacity) 
!					
!					! Copy the left half of the level to the right.
!					Do j = Levels, 2, -1
!						tree(2**(j-2) * 3: 2**j - 1) = tree(2**(j-1): 2**(j-2) * 3 - 1)
!					End Do
!
!					! Handle the top level separately.
!					tree(1)%right = tree(1)%left
!
!					FirstSpace = Capacity + 1.
!
!					! The sample volume is doubled to reflect
!					! the doubling of the ensemble
!					NDBLE = NDBLE + 1
!
!					! The tree now has one too many particles in it for
!					! use so we select a position at random from the
!					! uniform distribution and remove it, this
!					! corresponds to a reduction in sample volume.
!					
!					j = IRnd(Capacity) ! (uniform random int on [1,2**levels]).
!
!					! Remove particle and note scaling change.
!					Call NoteContraction(j, .True.)
!					err = RemoveParticleND(j)
!					FirstSpace = Capacity
				Else 
